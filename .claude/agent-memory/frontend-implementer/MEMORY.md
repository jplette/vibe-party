...
- Stack: Keycloakify v11 + React 18 + Vite 6 + `@radix-ui/themes` v3 + `@radix-ui/react-icons` v1
...
- Docker: `./keycloak-theme/dist_keycloak:/opt/keycloak/providers:ro` volume on keycloak service
- Realm: `"loginTheme": "vibe-party"` in `keycloak/realm-export.json` top level
- Makefile targets: `sync-theme-assets`, `build-theme`, `theme-storybook`

**Font bundling in Keycloakify (confirmed pattern):**
- Fonts live in TWO places: `public/fonts/lato/` (dev server) AND `src/styles/fonts/*.woff2` (Vite bundle)
- `_fonts.css` uses relative paths `./fonts/lato-v25-*.woff2` — Vite rewrites these into hashed asset URLs in the JAR
- Absolute paths like `/fonts/lato/...` break in the JAR (Keycloak serves theme resources at a different base URL)
- `make sync-theme-assets` copies woff2 files to both locations from `frontend/public/fonts/lato/`
- `_variables.css` is standalone — do NOT overwrite from frontend (`frontend/src/styles/theme.css` is Radix-specific)

**UI components in keycloak-theme pages (use Radix UI directly — no wrapper components):**
- `Button type="submit"` — works for HTML POST; `loading` prop shows spinner and auto-disables
- `TextField.Root name="..."` — forwards `name` to underlying `<input>` for form POST
- Password show/hide toggle: bare `<button type="button">` inside `<TextField.Slot side="right">` with `EyeOpenIcon`/`EyeClosedIcon`
- `Callout.Root color="red|green"` + `Callout.Text` — error/success messages with `role="alert"`
- `Flex`, `Grid`, `Text`, `Code` from `@radix-ui/themes`; icons from `@radix-ui/react-icons`
- `VibePartyProvider` — wraps with `<Theme accentColor="orange" grayColor="slate" radius="medium">`, imports Radix styles then `theme.css`

**CRITICAL: "Remember Me" in Keycloak forms:**
- Radix `<Switch>` does NOT emit a `name` attribute in HTML POST submissions
- Pattern: controlled `useState` + `<Switch checked onCheckedChange>` + `<input type="hidden" name="rememberMe" value={rememberMe ? "on" : ""}>`
- Initial state from `login.rememberMe === "on"` (string, not boolean)

### Deep-link / Auth Redirect Pattern (confirmed working)
- `AuthGuard` captures `location.pathname + location.search` via `useLocation()` and passes it as `returnTo` to `login(returnTo)`
- `login(returnTo?)` in `AuthProvider` calls `userManager.signinRedirect({ state: returnTo ?? '/' })`
- `oidc-client-ts` preserves `state` through the PKCE round-trip; returned on `User.state` after `signinRedirectCallback()`
- `CallbackPage` reads `user.state`, validates it starts with `/`, and navigates there; falls back to `/dashboard`
- Landing page / nav `onClick={login}` must be wrapped: `onClick={() => login()}` — passing `login` directly causes TS error because `returnTo` param collides with `MouseEvent` arg
