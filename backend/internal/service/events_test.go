...
func TestEventService_CreateEvent(t *testing.T) {
	...
	t.Run("creates event with valid name", func(t *testing.T) {
		...
		svc := newTestEventService(&mockEventRepo{
			CreateFn: func(_ context.Context, name, _, _ string, _ *string, _ uuid.UUID) (*model.Event, error) {
				return wantEvent, nil
			},
		})

		got, err := svc.CreateEvent(context.Background(), "Birthday Party", "", "", nil, creatorID)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("returns ErrInvalidInput when name is empty", func(t *testing.T) {
		...
		_, err := svc.CreateEvent(context.Background(), "", "", "", nil, creatorID)
	...
	t.Run("propagates repo error", func(t *testing.T) {
		...
		svc := newTestEventService(&mockEventRepo{
			CreateFn: func(_ context.Context, _, _, _ string, _ *string, _ uuid.UUID) (*model.Event, error) {
				return nil, repoErr
			},
		})

		_, err := svc.CreateEvent(context.Background(), "My Event", "", "", nil, creatorID)
		if err == nil {
			t.Fatal("expected error, got nil")
		}
	})
...
func TestEventService_UpdateEvent(t *testing.T) {
	...
	t.Run("updates event when caller is admin", func(t *testing.T) {
		...
		svc := newTestEventService(&mockEventRepo{
			...
			UpdateFn: func(_ context.Context, _ uuid.UUID, name, _, _ string, _ *string) (*model.Event, error) {
				updated.Name = name
				return updated, nil
			},
		})

		got, err := svc.UpdateEvent(context.Background(), eventID, adminID, "Updated Name", "", "", nil)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("returns ErrInvalidInput when name is empty", func(t *testing.T) {
		...
		_, err := svc.UpdateEvent(context.Background(), eventID, adminID, "", "", "", nil)
		if !errors.Is(err, ErrInvalidInput) {
			t.Errorf("got %v; want ErrInvalidInput", err)
		}
	})
	...
	t.Run("returns ErrForbidden when caller is not admin", func(t *testing.T) {
		...
		_, err := svc.UpdateEvent(context.Background(), eventID, memberID, "New Name", "", "", nil)
		if !errors.Is(err, ErrForbidden) {
			t.Errorf("got %v; want ErrForbidden", err)
		}
	})
	...
	t.Run("returns ErrForbidden when caller is not a member at all", func(t *testing.T) {
		...
		_, err := svc.UpdateEvent(context.Background(), eventID, uuid.New(), "New Name", "", "", nil)
		if !errors.Is(err, ErrForbidden) {
			t.Errorf("got %v; want ErrForbidden", err)
		}
	})
	...
	t.Run("returns ErrNotFound when event does not exist", func(t *testing.T) {
		svc := newTestEventService(&mockEventRepo{
			...
			UpdateFn: func(_ context.Context, _ uuid.UUID, _, _, _ string, _ *string) (*model.Event, error) {
				return nil, repository.ErrNotFound
			},
		})

		_, err := svc.UpdateEvent(context.Background(), eventID, adminID, "New Name", "", "", nil)
		if !errors.Is(err, ErrNotFound) {
			t.Errorf("got %v; want ErrNotFound", err)
		}
	})
